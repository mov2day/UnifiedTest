...
/**
 * Spock adapter. Spock 2 runs on the JUnit Platform, so it can use the shared JUnit Platform listener.
 */
public class SpockAdapter implements TestFrameworkAdapter {
    @Override
    public boolean isApplicable(Project project) {
        return FrameworkDetector.detect(project).contains("Spock");
    }
    ...
    public void registerListeners(Project project, Test testTask, UnifiedTestResultCollector collector, ConsoleReporter reporter, String theme) {
        ...
        project.getLogger().lifecycle("UnifiedTest: Gradle adapter for Spock registered");
    }
